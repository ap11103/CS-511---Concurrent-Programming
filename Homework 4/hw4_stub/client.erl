...
do_join(State, Ref, ChatName) ->
    case maps:is_key(ChatName, State#cl_st.con_ch) of
		true -> {err, State};
		false ->
			%% Notify the server to join the chatroom.
			whereis(server)!{self(), Ref, join, ChatName},
			receive 
				{ChatPID, Ref, connect, History} -> {History, State#cl_st{
...
do_leave(State, Ref, ChatName) ->
    case maps:find(ChatName, State#cl_st.con_ch) of
		{ok, _ChatPID} ->
			%% Notify the server to leave the chatroom.
			whereis(server)!{self(), Ref, leave, ChatName},
			receive
				{_PID, _Ref, ack_leave} ->
					%% Update the state to remove the chatroom from `con_ch`.
					UpdatedState = State#cl_st{
...
do_new_nick(State, Ref, NewNick) ->
    case NewNick == State#cl_st.nick of
		true -> {err_same, State};
		false ->
			%% Notify the server about the nickname change.
...
do_msg_send(State, Ref, ChatName, Message) ->
    ChatroomID = maps:get(ChatName, State#cl_st.con_ch),
	ChatroomID!{self(), Ref, message, Message},
    receive
        {ChatroomID, Ref, ack_msg} -> {{msg_sent, State#cl_st.nick}, State};
        _ -> {err, State}
    end.
